...
def generate_image(tide_data, photo_data=None):
    ...
    # ── HEADER ──────────────────────────────────────────────────────────────
    draw.rectangle([(0,0),(W,100)], fill=(12,18,30))
    draw.rectangle([(0,98),(W,102)], fill=BORDER)
    draw.text((40, 15), "UK TIDE TIMES", font=font(44, True), fill=TITLE_COL)
    draw.text((40, 66), datetime.now().strftime("%A  %d %B %Y").upper(), font=font(20), fill=MUTED)
    draw.text((W-40, 72), "ADMIRALTY DATA", font=font(14), fill=(50,80,110), anchor="ra")
    ...
    for idx, d in enumerate(tide_data):
        ...
        # Station name
        name = d["name"].upper()
        draw.text((cx+18, cy+16), name, font=font(28, True), fill=TEXT)

        # Divider under name
        draw.line([(cx+12, cy+58),(cx+cw-12, cy+58)], fill=BORDER2, width=1)

        if d["error"]:
            draw.text((cx+18, cy+76), "Data unavailable", font=font(24), fill=MUTED)
            continue

        # Two columns: High left, Low right
        mid = cx + cw // 2
        draw.line([(mid, cy+58),(mid, cy+ch-12)], fill=BORDER2, width=1)

        # HIGH column (left)
        hx = cx + 18
        yo = cy + 70
        draw.text((hx, yo), "HIGH", font=font(26, True), fill=HI_COL)
        draw.text((hx, yo+32), "TIDES", font=font(26, True), fill=HI_COL)
        yo += 82
        if d["highs"]:
            for h in d["highs"][:2]:
                t = h["time"].replace(" BST","").replace(" GMT","")
                draw.text((hx, yo), t, font=font(48, True), fill=TEXT)
                draw.text((hx, yo+56), f"{h['height']:.2f}m", font=font(28), fill=HI_COL)
                yo += 96
        else:
            draw.text((hx, yo), "None today", font=font(24), fill=MUTED)

        # LOW column (right)
        lx = mid + 18
        yo = cy + 70
        draw.text((lx, yo), "LOW", font=font(26, True), fill=LO_COL)
        draw.text((lx, yo+32), "TIDES", font=font(26, True), fill=LO_COL)
        yo += 82
        if d["lows"]:
            for l in d["lows"][:2]:
                t = l["time"].replace(" BST","").replace(" GMT","")
                draw.text((lx, yo), t, font=font(48, True), fill=TEXT)
                draw.text((lx, yo+56), f"{l['height']:.2f}m", font=font(28), fill=LO_COL)
                yo += 96
        else:
            draw.text((lx, yo), "None today", font=font(24), fill=MUTED)
