...
def generate_image(tide_data, photo_data=None):
    ...
    for idx, d in enumerate(tide_data):
        ...
        draw.rectangle([(cx,cy),(cx+6,cy+ch)], fill=BORDER)

        # Station name
        name = d["name"].upper()
        draw.text((cx+16, cy+16), name, font=font(30, True), fill=TEXT)

        # Divider under name
        draw.line([(cx+10, cy+62),(cx+cw-10, cy+62)], fill=BORDER2, width=1)

        if d["error"]:
            draw.text((cx+16, cy+80), "Data unavailable", font=font(24), fill=MUTED)
            continue

        # Two columns: High left, Low right
        mid = cx + cw // 2
        draw.line([(mid, cy+62),(mid, cy+ch-10)], fill=BORDER2, width=1)
