...
def generate_image(tide_data, photo_data=None):
    ...
    for idx, d in enumerate(tide_data):
        ...
        # Station name
        name = d["name"].upper()
        draw.text((cx+18, cy+16), name, font=font(16, True), fill=TEXT)

        # Divider under name
        draw.line([(cx+12, cy+44),(cx+cw-12, cy+44)], fill=BORDER2, width=1)

        if d["error"]:
            draw.text((cx+18, cy+60), "Data unavailable", font=font(14), fill=MUTED)
            continue

        # Two columns: High left, Low right
        mid = cx + cw // 2
        draw.line([(mid, cy+44),(mid, cy+ch-12)], fill=BORDER2, width=1)

        # HIGH column (left)
        hx = cx + 18
        yo = cy + 54
        draw.text((hx, yo), "HIGH TIDES", font=font(12, True), fill=HI_COL)
        yo += 22
        if d["highs"]:
            for h in d["highs"][:2]:
                t = h["time"].replace(" BST","").replace(" GMT","")
                draw.text((hx, yo), t, font=font(26, True), fill=TEXT)
                draw.text((hx, yo+30), f"{h['height']:.2f}m", font=font(16), fill=HI_COL)
                yo += 54
        else:
            draw.text((hx, yo), "None today", font=font(14), fill=MUTED)

        # LOW column (right)
        lx = mid + 18
        yo = cy + 54
        draw.text((lx, yo), "LOW TIDES", font=font(12, True), fill=LO_COL)
        yo += 22
        if d["lows"]:
            for l in d["lows"][:2]:
                t = l["time"].replace(" BST","").replace(" GMT","")
                draw.text((lx, yo), t, font=font(26, True), fill=TEXT)
                draw.text((lx, yo+30), f"{l['height']:.2f}m", font=font(16), fill=LO_COL)
                yo += 54
        else:
            draw.text((lx, yo), "None today", font=font(14), fill=MUTED)
