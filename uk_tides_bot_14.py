...
def generate_image(tide_data, photo_data=None):
    ...
    for idx, d in enumerate(tide_data):
        ...
        name = d["name"].upper()
        draw.text((cx+14, cy+12), name, font=font(14, True), fill=TEXT)

        if d["error"]:
            draw.text((cx+14, cy+42), "Data unavailable", font=font(13), fill=MUTED)
            continue
        ...
        draw.text((cx+cw-10, cy+13), tsym, font=font(12), fill=tcol, anchor="ra")

        # Divider
        draw.line([(cx+12, cy+36),(cx+cw-12, cy+36)], fill=BORDER2, width=1)

        # Two columns: High left, Low right
        mid = cx + cw // 2
        draw.line([(mid, cy+36),(mid, cy+ch-10)], fill=BORDER2, width=1)

        # HIGH column (left)
        hx = cx + 14
        yo = cy + 44
        draw.text((hx, yo), "HIGH TIDES", font=font(11, True), fill=HI_COL)
        yo += 18
        if d["highs"]:
            for h in d["highs"][:2]:
                t = h["time"].replace(" BST","").replace(" GMT","")
                draw.text((hx, yo), t, font=font(19, True), fill=TEXT)
                draw.text((hx, yo+22), f"{h['height']:.2f}m", font=font(13), fill=HI_COL)
                yo += 40
        else:
            draw.text((hx, yo), "None today", font=font(13), fill=MUTED)

        # LOW column (right)
        lx = mid + 14
        yo = cy + 44
        draw.text((lx, yo), "LOW TIDES", font=font(11, True), fill=LO_COL)
        yo += 18
        if d["lows"]:
            for l in d["lows"][:2]:
                t = l["time"].replace(" BST","").replace(" GMT","")
                draw.text((lx, yo), t, font=font(19, True), fill=TEXT)
                draw.text((lx, yo+22), f"{l['height']:.2f}m", font=font(13), fill=LO_COL)
                yo += 40
        else:
            draw.text((lx, yo), "None today", font=font(13), fill=MUTED)
