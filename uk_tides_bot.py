...
def get_tide_predictions(station_id, station_name):
    ...
    try:
        ...
        for event in events:
            ...
            try:
                # Admiralty API returns GMT with no timezone suffix
                # Parse as naive datetime then apply BST manually
                dt = datetime.fromisoformat(dt_str.split(".")[0])  # strip fractional seconds
                # Apply BST conversion (last Sun Mar to last Sun Oct = +1hr)
