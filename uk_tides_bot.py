...
def get_tide_predictions(station_id, station_name):
    ...
    try:
        ...
        # Simple trend: if next event is high tide we are rising, else falling
        all_events = sorted(
            [(h["time"], "High") for h in result["highs"]] +
            [(l["time"], "Low") for l in result["lows"]],
            key=lambda x: x[0]
        )
...
def generate_image(tide_data, photo_data=None):
    ...
    # ── HEADER ──────────────────────────────────────────────────────────────
    draw.rectangle([(0,0),(W,120)], fill=(12,18,30))
    # Teal accent line
    draw.rectangle([(0,118),(W,122)], fill=BORDER)
    # Logo / title
    draw.text((44, 18), "TIDE", font=font(52, True), fill=TITLE_COL)
    draw.text((160, 18), "TIMES", font=font(52, False), fill=TEXT)
    # Date
    draw.text((44, 78), datetime.now().strftime("%A  ·  %d %B %Y").upper(), font=font(18), fill=MUTED)
    # Wave decoration
    for i in range(8):
        x = W - 240 + i * 26
        draw.arc([(x-10, 55), (x+10, 75)], 180, 0, fill=(30,50,78), width=2)
    # Admiralty badge
    badge_x = W - 40
    draw.text((badge_x, 88), "ADMIRALTY DATA", font=font(11), fill=(50,80,110), anchor="ra")
    ...
    for idx, d in enumerate(tide_data):
        ...
        # Card background with subtle inner border
        draw.rounded_rectangle([(cx,cy),(cx+cw,cy+ch)], radius=8, fill=SURFACE)
        draw.rounded_rectangle([(cx,cy),(cx+cw,cy+ch)], radius=8, outline=BORDER2, width=1)
        # Left accent bar
        draw.rounded_rectangle([(cx,cy),(cx+5,cy+ch)], radius=4, fill=BORDER)

        # Station name
        name = d["name"]
        draw.text((cx+18, cy+14), name.upper(), font=font(16, True), fill=TEXT)

        if d["error"]:
            draw.text((cx+18, cy+50), "Data unavailable", font=font(14), fill=MUTED)
            continue

        # Trend indicator (top right)
        trend = d.get("trend", "")
        if trend == "Rising":
            tcol, tsym = TREND_UP, "▲ Rising"
        elif trend == "Falling":
            tcol, tsym = TREND_DN, "▼ Falling"
        else:
            tcol, tsym = TREND_NT, "► Slack"
        draw.text((cx+cw-12, cy+16), tsym, font=font(13), fill=tcol, anchor="ra")

        # Divider
        draw.line([(cx+14, cy+42),(cx+cw-14, cy+42)], fill=BORDER2, width=1)

        # High tides column
        col_x = cx + 18
        yo = cy + 52
        draw.text((col_x, yo), "⬆  HIGH TIDES", font=font(13, True), fill=HI_COL)
        yo += 22
        if d["highs"]:
            for h in d["highs"][:2]:
                # Time (large) + height (small)
                draw.text((col_x, yo), h["time"], font=font(20, True), fill=TEXT)
                draw.text((col_x + 120, yo + 4), f"{h['height']:.2f}m", font=font(15), fill=HI_COL)
                yo += 26
        else:
            draw.text((col_x, yo), "None yet today", font=font(14), fill=MUTED)
            yo += 26

        # Low tides column
        yo += 4
        draw.text((col_x, yo), "⬇  LOW TIDES", font=font(13, True), fill=LO_COL)
        yo += 22
        if d["lows"]:
            for l in d["lows"][:2]:
                draw.text((col_x, yo), l["time"], font=font(20, True), fill=TEXT)
                draw.text((col_x + 120, yo + 4), f"{l['height']:.2f}m", font=font(15), fill=LO_COL)
                yo += 26
        else:
            draw.text((col_x, yo), "None yet today", font=font(14), fill=MUTED)
    ...
    draw.text((44, foot_y+16), "Plan your coastal day safely", font=font(15), fill=MUTED)
    draw.text((W-44, foot_y+16), "#UKTides  #Cornwall  #Devon  #CoastalLife", font=font(14), fill=(50,80,110), anchor="ra")
