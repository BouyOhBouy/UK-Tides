...
def generate_image(tide_data, photo_data=None):
    ...
    for idx, d in enumerate(tide_data):
        ...
        draw.rounded_rectangle([(cx,cy),(cx+cw,cy+ch)], radius=10, outline=BORDER2, width=1)
        draw.rounded_rectangle([(cx,cy),(cx+5,cy+ch)], radius=6, fill=BORDER)

        # Station name
        name = d["name"].upper()
        draw.text((cx+18, cy+20), name, font=font(28, True), fill=TEXT)

        # Divider under name
        draw.line([(cx+12, cy+62),(cx+cw-12, cy+62)], fill=BORDER2, width=1)

        if d["error"]:
            draw.text((cx+18, cy+80), "Data unavailable", font=font(22), fill=MUTED)
            continue

        # Two columns: High left, Low right
        mid = cx + cw // 2
        draw.line([(mid, cy+62),(mid, cy+ch-12)], fill=BORDER2, width=1)

        # HIGH column (left)
        hx = cx + 18
        yo = cy + 74
        draw.text((hx, yo), "HIGH TIDES", font=font(46, True), fill=HI_COL)
        yo += 56
        if d["highs"]:
            for h in d["highs"][:2]:
                t = h["time"].replace(" BST","").replace(" GMT","")
                draw.text((hx, yo), t, font=font(46, True), fill=TEXT)
                draw.text((hx, yo+52), f"{h['height']:.2f}m", font=font(24), fill=HI_COL)
                yo += 90
        else:
            draw.text((hx, yo), "None today", font=font(22), fill=MUTED)

        # LOW column (right)
        lx = mid + 18
        yo = cy + 74
        draw.text((lx, yo), "LOW TIDES", font=font(46, True), fill=LO_COL)
        yo += 56
        if d["lows"]:
            for l in d["lows"][:2]:
                t = l["time"].replace(" BST","").replace(" GMT","")
                draw.text((lx, yo), t, font=font(46, True), fill=TEXT)
                draw.text((lx, yo+52), f"{l['height']:.2f}m", font=font(24), fill=LO_COL)
                yo += 90
        else:
            draw.text((lx, yo), "None today", font=font(22), fill=MUTED)
