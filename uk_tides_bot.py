...
def generate_image(tide_data, photo_data=None):
    ...
    for idx, d in enumerate(tide_data):
        ...
        # Station name
        name = d["name"].upper()
        draw.text((cx+18, cy+14), name, font=font(22, True), fill=TEXT)

        # Divider under name
        draw.line([(cx+12, cy+48),(cx+cw-12, cy+48)], fill=BORDER2, width=1)

        if d["error"]:
            draw.text((cx+18, cy+66), "Data unavailable", font=font(20), fill=MUTED)
            continue

        # Two columns: High left, Low right
        mid = cx + cw // 2
        draw.line([(mid, cy+48),(mid, cy+ch-12)], fill=BORDER2, width=1)

        # HIGH column (left)
        hx = cx + 18
        yo = cy + 58
        # Label
        draw.text((hx, yo), "HIGH", font=font(20, True), fill=HI_COL)
        draw.text((hx, yo+26), "TIDES", font=font(20, True), fill=HI_COL)
        yo += 66
        if d["highs"]:
            for h in d["highs"][:2]:
                t = h["time"].replace(" BST","").replace(" GMT","")
                draw.text((hx, yo), t, font=font(34, True), fill=TEXT)
                draw.text((hx, yo+40), f"{h['height']:.2f}m", font=font(20), fill=HI_COL)
                yo += 72
        else:
            draw.text((hx, yo), "None today", font=font(18), fill=MUTED)

        # LOW column (right)
        lx = mid + 18
        yo = cy + 58
        # Label
        draw.text((lx, yo), "LOW", font=font(20, True), fill=LO_COL)
        draw.text((lx, yo+26), "TIDES", font=font(20, True), fill=LO_COL)
        yo += 66
        if d["lows"]:
            for l in d["lows"][:2]:
                t = l["time"].replace(" BST","").replace(" GMT","")
                draw.text((lx, yo), t, font=font(34, True), fill=TEXT)
                draw.text((lx, yo+40), f"{l['height']:.2f}m", font=font(20), fill=LO_COL)
                yo += 72
        else:
            draw.text((lx, yo), "None today", font=font(18), fill=MUTED)
